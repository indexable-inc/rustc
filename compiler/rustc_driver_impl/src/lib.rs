...
use rustc_session::config::{
    ...
    SwitchWithOptPath, Sysroot, UnstableOptions, Z_OPTIONS, nightly_options, parse_target_triple,
};
...
use crate::session_diagnostics::{
    CantDumpTestNames, CantEmitMIR, RLinkEmptyVersionNumber, RLinkEncodingVersionMismatch,
    RLinkRustcVersionMismatch, RLinkWrongFileType, RlinkCorruptFile, RlinkNotAFile,
    RlinkUnableToRead, UnstableFeatureUsage,
};
...
pub fn run_compiler(at_args: &[String], callbacks: &mut (dyn Callbacks + Send)) {
    ...
    interface::run_compiler(config, |compiler| {
        ...
        let linker = create_and_enter_global_ctxt(compiler, krate, |tcx| {
            ...
            // `-Zdump-test-names`: the `#[test]` harness has been generated by now, so
            // the full test list is known. Report it and stop before analysis, codegen
            // and linking, which is the whole point of the flag.
            if let SwitchWithOptPath::Enabled(path) = &sess.opts.unstable_opts.dump_test_names
                && sess.is_test_crate()
            {
                dump_test_names(tcx, path.as_deref());
                return None;
            }
...
/// Write the `#[test]`/`#[bench]` functions collected by the test harness as JSON,
/// for `-Zdump-test-names`.
///
/// The list is exactly the one the generated harness registers, in the same order,
/// so `tests[*].name` matches what the linked test binary prints for
/// `--list --format terse` (which formats each entry as `<name>: <kind>`).
fn dump_test_names(tcx: TyCtxt<'_>, path: Option<&Path>) {
    use rustc_session::parse::{CollectedTestType, TestShouldPanic};
    ...
    let tests: Vec<_> = tcx
        ...
        .values()
        .map(|test| {
            let (should_panic, should_panic_message) = match test.should_panic {
                TestShouldPanic::No => ("no", None),
                TestShouldPanic::Yes => ("yes", None),
                TestShouldPanic::YesWithMessage(msg) => ("yes", Some(msg.to_string())),
            };
            json!({
                "name": test.name.to_string(),
                // The strings libtest's `--list` uses for the same distinction, so
                // that `"{name}: {kind}"` reproduces a `--format terse` line.
                "kind": if test.is_bench { "benchmark" } else { "test" },
                "ignore": test.ignore,
                "ignore_message": test.ignore_message.map(|msg| msg.to_string()),
                "should_panic": should_panic,
                "should_panic_message": should_panic_message,
                "test_type": match test.test_type {
                    CollectedTestType::UnitTest => "unit",
                    CollectedTestType::IntegrationTest => "integration",
                    CollectedTestType::Unknown => "unknown",
                },
                "source_file": test.source_file.to_string(),
                "start_line": test.start_line,
                "start_col": test.start_col,
    ...
    let mut rendered = serde_json::to_string_pretty(&doc).unwrap();
    rendered.push('\n');

    match path {
        None => safe_print!("{rendered}"),
        Some(path) => {
            if let Err(error) = fs::write(path, rendered) {
                tcx.dcx().emit_err(CantDumpTestNames { path, error });
            }
        }
    }
}
